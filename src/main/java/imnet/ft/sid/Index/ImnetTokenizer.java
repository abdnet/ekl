...
public class ImnetTokenizer {
	...
	public XContentBuilder getTokenizerXContent(XContentBuilder tokenizer) throws IOException {
			if(this.isDefaut) {
				if( !this.tokenizer_Name.equals("")&&ElasticSearchReservedFilters.tokenizer.contains(this.tokenizer_Name.toLowerCase())) {
							...
							if(!this.tokenizerCreated.contains(entry.getKey()) && entry.getValue()!=null){	
								...
								for(Entry<String,Object> entry2:option.entrySet()) {
									if(entry2.getKey().equals("type")) {
										if(ElasticSearchReservedFilters.tokenizer.contains(entry2.getValue())) {
								...
								if(!this.tokenizerCreated.contains(entry.getKey())&&ElasticSearchReservedFilters.tokenizer.contains(entry.getKey()))
									this.tokenizerCreated.add(entry.getKey());
								else
									continue;
								}
